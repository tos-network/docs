...
description: Complete API documentation for building applications on TOS Network with privacy, smart contracts, and mining
---
...
- ⛏️ **Mining Integration**: Participate in PoW mining
- ⚡ **Energy Management**: Gas-free transactions through staking
- 🚀 **Smart Contracts**: Rust smart contracts with TAKO runtime
...
/* Returns:
{
  ...
  "network_difficulty": {
    "gpu_mining": "0x1a2b3c4d",
    "cpu_mining": "0x5e6f7a8b"
  }
}
*/
```
...
const deployResult = await tos.contracts.deploy({
    bytecode: contractBytecode, // Compiled TAKO bytecode (.wasm)
...
/* Returns:
{
  ...
  "mining": {
    "gpu_mining_percentage": 12.0,
    "cpu_mining_percentage": 88.0,
...
async def start_mining():
    # Register as miner
    await tos.mining.register_miner({
        'hardware_profile': {
            'cpu_cores': 8,
            'ram_gb': 16,
            'gpu_available': True
...
| `MINING_TIMEOUT` | Mining task expired | Submit solutions faster |
...
tos-node --dev --port 8080 --enable-mining
...
- [Smart Contract Development](/getting-started/smart-contract-development) - Build Rust smart contracts
- [Privacy Integration Guide](/developers-api/privacy-integration) - Implement privacy features
- [Mining API Reference](/developers-api/stratum) - Mining pool protocol
